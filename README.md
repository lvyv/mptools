**OptiDock系统指南**
====================
![img](docs/images/optidock.webp)
集装箱码头作业现场存在人、车、箱、船、机、场等生产力要素，这些要素需要被高效的组织起来以提升港口生产效率。OptiDock是一个利用机器视觉网络技术采集港口区域内生产力要素目标的时空轨迹和状态事件以提升码头作业效率的软件框架。它最初被用于外集卡数据无接触采集，后续可用于识别码头其它生产力要素目标。OptiDock支持分布式部署，从而采集主流NVR，IPC监控设备获取rtsp流解码，调度AI算法模型，最终实现在线实时目标追踪和状态识别的功能效果。  
