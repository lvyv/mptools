...
from core.kernel import FSM, ProcSimpleFactory, MainContext


class MeterMainContext(MainContext):
    """封装主进程模块类.
    完成所有子进程的创建，终止工作.
    ...
    def fork_work_process(self, expect_rtsp_num, now_rtsp_num, now_ai_num, now_mqtt_num) -> bool:
        ...
        for _ in range(_need_rtsp_num):
            self.__switch_on_process('RtspWorker')    # RTSP
        for _ in range(_need_ai_num):
            self.__switch_on_process('AiWorker')      # AI

        mqtt = _v2v_cfg_dict['mqtt_svrs'][0]
        for _ in range(_need_mqtt_num):         # MQTT
            self.__switch_on_process('MqttWorker', mqtt_host=mqtt['mqtt_svr'], mqtt_port=mqtt['mqtt_port'],
    ...
    def show_queues_status(self):
        ...
        Returns:
            None
        """
        self.log(f'[{self.__class__.__name__}] placeholder function.', level=log.LOG_LVL_INFO)
    ...
    def run(self, rest='core.rest.RestWorker', cfg=''):
        try:
            # 读取配置文件内容
            if cfg:
            ...
            self.fork_restful_process(**_v2v_cfg_dict)
            # 进入主循环，阻塞处理子进程之间的消息.
